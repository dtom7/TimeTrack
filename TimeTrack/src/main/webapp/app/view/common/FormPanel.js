/**
 * This is a Form Panel that allows for simplified integration with Models. It
 * can be instantiated in one of three ways:
 * 
 * Create empty record Ext.create('TT.view.common.FormPanel', { model:
 * 'MC.model.MyModel' });
 * 
 * Modify an existing record for which you only have the id
 * Ext.create('TT.view.common.FormPanel', { model: 'MC.model.MyModel', modelId:
 * 1234 });
 * 
 * Modify a record for which you have an existing model
 * Ext.create('TT.view.common.FormPanel', { model: myModelInstance });
 * 
 * Invoking myFormPanel.commit() will save the record forcing the data to be
 * written if a proxy (with a writer) is defined on the Model.
 */
Ext.define('TT.view.common.FormPanel', {
	...
	loadRecord : function(model) {
		console.log('In FormPanel loadRecord');
		...
		for (i = 0, len = associations.length; i < len; i++) {
			...
			console.log('In FormPanel association: ' + name);
